    ...
    <div>
        <div className="mx-auto max-w-2xl  px-4 py-16 sm:px-6 sm:py-24 lg:grid lg:max-w-7xl lg:grid-cols-2  lg:gap-x-8 lg:py-32 xl:gap-x-24">
            <div className="lg:col-start-2">
                ...
                <h1 className="mt-2 text-4xl font-bold  tracking-tighter text-gray-900 sm:text-5xl">Thanks for ordering</h1>
                {order._isPaid ? <p className="mt-2 text-base text-muted-foreground">
                    Your order was processed and your assets are available to dowmload below. We&apos;ve sent your receipt and order details to{' '} {typeof order.user !=="string" ? <span className="font-medium text-gray-900">{order.user.email}</span> : null }.
                </p> : (<p className="mt-2 text-base text-muted-foreground">
                    We appreciate your order , and we&apos;re currently processing it. So hang tight we&apos;ll send you confirmation very soon!
                    </p>)}
                                ...
                                return <li key={product.id} className="flex space-x-6 py-6">
                                    <div className="relative  h-24 w-24 ">
                                        {typeof  image !== "string" && image.url ? (
                                            <Image fill  src={image.url} alt={`${product.name} image`} className="flex-none rouded-md bg-gray-100 object-cover object-center"/>
                                        ) : null}
                                    </div>
